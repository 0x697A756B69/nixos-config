# Application launcher (SUPER+R), ported from Elpu7/dotfiles
# (github.com/Elpu7/dotfiles, .config/walker + .config/hypr/hyprland.lua)
# to replace wofi. config.toml is copied verbatim from that repo (theme
# name aside); the CSS keeps every non-color rule as-is and swaps the
# hardcoded Tokyonight @define-color block for our matugen palette.
{ config, pkgs, ... }:

{
  ...
  xdg.configFile."walker/config.toml".text = ''
    ...
    resume_last_query = true          # open walker with the last query in place
    actions_as_menu = false            # display all possible actions in a submenu
    autoplay_videos = false            # auto-play video previews
    ext_background_effect_blur = true # request the compositor to blur behind the box-wrapper via ext-background-effect-v1 (compositor must support it)
    ...
    max_results = 20 # global max results
    ...
    archlinuxpkgs = [
      ...
      { action = "remove", bind = "Return", default = true },
    ...
    clipboard = [
      ...
      { action = "show_pinned_only", label = "only pinned", bind = "ctrl i", after = "AsyncClearReload" },
  ...
  # Same selectors/radii/paddings as Elpu7's elpu7-tokyonight theme; only
  # the @define-color block is swapped for our matugen palette (same
  # @import approach as modules/home/wofi) and the font-family matches
  # what's actually installed here.
  xdg.configFile."walker/themes/matugen/style.css".text = ''
    ...
    * {
    ...
    .box-wrapper {
        ...
        border-radius: 18px;
        border: 1px solid alpha(@border, 0.20);
